...
export default function Areamodal({
  ...
  const [selectapplicationBaseOptions, setSelectapplicationBaseOptions] =
    useState<IGRPOptionsProps[]>([]);
  const [selectparentIdOptions, setSelectparentIdOptions] = useState<
    IGRPOptionsProps[]
  >([]);
    ...
    <div className={cn("component")}>
      <IGRPModalDialog onOpenChange={setOpen} open={open}>
        <IGRPModalDialogContent size={`lg`} className={cn()}>
          ...
          >
            <>
              {selectapplicationBaseOptions.length > 0 && (
                <IGRPCombobox
                  id={`applicationBase`}
                  label={`Aplicação`}
                  variant={`single`}
