...
import { AreasList } from '@/app/(myapp)/process-configuration/components/areas-list'
import { ArtifactProcessesList } from '@/app/(myapp)/process-configuration/components/artifact-processes-list'
  ...
  return (
    <div className={cn('page', 'space-y-6',)}    >
        ...
        items={
          [
...

            {
              ...
              badgeColor: `primary`,
              content: (<>
                <ArtifactProcessesList processes={pc.allProcesses} onArtifactEdit={(processId) => pc.handleOpenArtifactModal(processId)}
                  onProcessNumberEdit={(processId, processKey) => pc.handleOpenProcessNumberModal(processId, processKey, '')}
                  onArtifactPermissions={(processId) =>
                    pc.handleOpenArtifactPermissionModal(processId)} ></ArtifactProcessesList>
              </>),
            },
          ]
        }
      ...
      <ProcessModal availableProcesses={pc.getAvailableProcesses(pc.processForm.modalState.selectedAreaId || '')} open={pc.processForm.modalState.isOpen} onAssociate={pc.handleAssociateProcess}
        ...
        setOpen={(open) => (open ? pc.processForm.openModal(pc.processForm.modalState.selectedAreaId || '') : pc.processForm.closeModal())} ></ProcessModal>
      <ArtifactModal open={pc.artifactForm.modalState.isOpen} processArtifacts={pc.processArtifacts} formData={pc.artifactForm.formData} setOpen={(open) => open ? pc.handleOpenArtifactModal(pc.artifactForm.modalState.selectedProcessId || '') : pc.artifactForm.closeModal()}
        onFormChange={pc.artifactForm.updateFormData}
        onSave={pc.handleSaveArtifacts} ></ArtifactModal>
      <ProcessConfigurationPermission formData={pc.artifactPermissionForm.formData} open={pc.artifactPermissionForm.modalState.isOpen} processArtifacts={pc.processArtifactsPermission} isSubmiting={pc.artifactPermissionForm.loading} onSave={pc.handleSaveArtifactPermission}
      ...
      <ProcessNumberModal open={pc.processNumberForm.modalState.isOpen} formData={pc.processNumberForm.formData} setOpen={(open) => open ? pc.handleOpenProcessNumberModal(pc.processNumberForm.modalState.selectedProcessId || '', pc.processNumberForm.modalState.selectedProcessKey || '', pc.processNumberForm.modalState.selectedProcessApplicationBase || '') : pc.processNumberForm.closeModal()}
        onFormChange={pc.processNumberForm.updateFormData}
        onSave={pc.handleSaveProcessNumber} ></ProcessNumberModal>
      <IGRPAlertDialog
        variant={`primary`}
        actionLabel={`Confirm`}
        cancelLabel={`Cancel`}
