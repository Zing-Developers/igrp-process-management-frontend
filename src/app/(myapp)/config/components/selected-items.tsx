...
export function SelectedItems({ items, removeItem }: { items: string, removeItem: (item: string) => void }) {

    const itemsArray = items ? items.split(',') : [];
    return (<div className="flex flex-wrap gap-2" >
        {
            itemsArray.length > 0 && itemsArray.map((group) => (
        ...
        {
            itemsArray.length === 0 && (
                <p className="text-sm text-muted-foreground">No candidate groups assigned</p>
            )
        }
    </div>
    )
