...
export const getM2mKeys = async (): Promise<
  M2mKeysActionResult<KeySummary[]>
> => {
  try {
    const client = await getIGRPProcessClient();
    const response = await client.m2mKeys.getKeys();
...
export const createM2mKey = async (
  ...
  try {
    const client = await getIGRPProcessClient();
    const response = await client.m2mKeys.createKey(request);
    return { success: true, data: response.data };
  } catch (error) {
    return getErrorResult(error);
  }
...
export const revokeM2mKey = async (
  ...
  try {
    const client = await getIGRPProcessClient();
    await client.m2mKeys.revokeKey(id);
...
export const rotateM2mKey = async (
  ...
  try {
    const client = await getIGRPProcessClient();
    const response = await client.m2mKeys.rotateKey(id);
    return { success: true, data: response.data };
  } catch (error) {
    return getErrorResult(error);
  }
