...
export default function Commonusertaskmodalform({ open, setOpen, onSave, modalTitle, modalSubTitle, userRequired, showPriority } : { open: boolean, setOpen: (prompt: boolean) => void, onSave: (data: any) => void, modalTitle: string, modalSubTitle: string, userRequired?: boolean, showPriority?: boolean }) {
...
const initForm1: z.infer<Form1ZodType> = {
  ...
  const [radioatribuirTaskOptions, setRadioatribuirTaskOptions] = useState<IGRPOptionsProps[]>([]);
...
formRef={ formform1Ref }
  className={ cn('',) }
  onSubmit={ async (data) => { onSave({...data, assigneTo:atribuirTask})} }
...
{ atribuirTask === 'group' && (<IGRPInputText
  id={ `candidateGroups` }
  label={ `Grupo` }
showIcon={ false }
required={ true }
