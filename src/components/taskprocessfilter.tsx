...
export default function Taskprocessfilter({ onSearch, onApplyFilters, onResetFilters, onFiltersChange, isProcess } : { onSearch: (searchTerm: string) => void, onApplyFilters: (filters: any) => void, onResetFilters: () => void, onFiltersChange: (filters: any) => void, isProcess?: boolean }) {
...
<div className={ cn('component',)}    >
	<div className={ cn('flex flex-row flex-wrap items-center justify-start gap-2',' px-3 flex-1',)}    >
	<IGRPInputSearch
  id={ `inputSearch1` }
  label={ undefined }
showStartIcon={ true }
...
iconName={ `CornerDownRight` }
  ...
  onChange={ (selected) => handleStatusChange(Array.isArray(selected) ? selected[0] : selected) }
...
<IGRPDatePickerRange
  ...
  className={ cn('col-span-1',) }
/>
<FilterData   onChange={ (filters) => handleFiltersChange(filters) } ></FilterData></div>
<IGRPButton
  id={ `button3` }
  variant={ `outline` }
size={ `default` }
...
{ showFilter && (<div className={ cn('grid','grid-cols-2 ','md:grid-cols-2 ','lg:grid-cols-4 ',' gap-4 px-3',)}    >
	<IGRPInputText
  id={ `Processnumber` }
...
iconName={ `CornerDownRight` }
  className={ cn('col-span-1',) }
  onChange={ (selected) => handleAreaChange(Array.isArray(selected) ? selected[0] : selected) }
...
iconName={ `CornerDownRight` }
  className={ cn('col-span-1',) }
  onChange={ (selected) => handleSubareaChange(Array.isArray(selected) ? selected[0] : selected) }
...
iconName={ `CornerDownRight` }
  className={ cn('col-span-1',) }
  onChange={ (selected) => handleUserChange(Array.isArray(selected) ? selected[0] : selected) }
...
{ showFilter && (<div className={ cn('flex','flex flex-row flex-nowrap items-stretch justify-end gap-2',' pb-3 px-3',)}    >
	<IGRPButton
  id={ `button1` }
  variant={ `outline` }
size={ `default` }
