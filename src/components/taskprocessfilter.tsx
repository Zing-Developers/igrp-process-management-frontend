...
export default function Taskprocessfilter({ onSearch, onApplyFilters, onResetFilters } : { onSearch: (searchTerm: string) => void, onApplyFilters: (filters: any) => void, onResetFilters: () => void }) {
...
<div className={ cn('component',)}    >
	<div className={ cn(' px-4 pt-2 flex-1 items-center',)}    >
	<div className={ cn('flex flex-row flex-wrap items-center justify-start gap-2',' flex-1',)}    >
	<IGRPInputSearch
  id={ `inputSearch1` }
  label={ undefined }
showStartIcon={ true }
...
<FilterData   onChange={ (filters) => handleFiltersChange(filters) } ></FilterData>
<IGRPDatePickerRange
...
{ showFilter && (<div className={ cn('grid','grid-cols-2 ','md:grid-cols-2 ','lg:grid-cols-4 ',' gap-4 px-4 pt-2',)}    >
	<IGRPInputText
  id={ `Processnumber` }
...
{ showFilter && (<div className={ cn('flex','flex flex-row flex-nowrap items-stretch justify-end gap-2',' px-4 pt-2 space-y-3',)}    >
	<IGRPButton
  id={ `button2` }
  variant={ `outline` }
size={ `default` }
